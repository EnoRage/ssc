...
contract('CarMarket', (accounts) => {
    ...
    describe('MAIN TEST', () => {
        it("should add new car to Market", async function () {
            let firstCar = cars[0];
            let carIndex = await carMarket.addCar(
                firstCar.price,
            ...
            let createdCar = await carMarket.cars(carIndex.logs[0].args.carID.toNumber());
    ...
    Car presence: ${createdCar[4]}
        ...
        it("should buy car", async function () {
            ...
            for (let i in buyers) {
                await carMarket.buyCar(tx.logs[0].args.carID, {value: firstCar.price, from: buyers[i]});
                let carOwnerID = await carMarket.carOwner(buyers.account2, {from: buyers[i]});
                let carOwner = await  carMarket.carOwners(carOwnerID, {from: buyers[i]});
                console.log(
