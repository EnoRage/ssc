...
contract Sale {
    ...
    modifier goodSale() {
        require(now > start && now < start + interval * 1 days); 
// проверяем, что функция вызвана в промежуток между стартом и окончанием 

        _;
    }

    modifier belowhardcap() {
        require(ethacc.balance <= hardcap); 
// смотрим, когда баланс на нужном аккаунте станет hardcap и потом выкидываем
        _;
    }
